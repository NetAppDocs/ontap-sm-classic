...
[.lead]
You can use ONTAP System Manager classic (available in 9.7 and earlier) if you prefer to use a graphic interface instead of the command-line interface for accessing and managing a cluster. ONTAP System Manager is included with ONTAP as a web service, is enabled by default, and is accessible by using a browser.
