...
[.lead]
You can set up Security Assertion Markup Language (SAML) authentication by using ONTAP System Manager classic (available in ONTAP 9.7 and earlier). Remote users are authenticated through a secure identity provider (IdP) before they log in to System Manager.
